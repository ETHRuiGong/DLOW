...
class Visualizer():
    ...
    def print_current_losses(self, epoch, i, losses, t, t_data):
        ...
        for k, v in losses.items():
            message += '%s: %.3f ' % (k, v.data.clone().cpu().numpy().tolist()[0])
    ...
    def save_images_withlabel(self, webpage, visuals, image_path, aspect_ratio=1.0):
        image_dir = webpage.get_image_dir()
        short_path = ntpath.basename(image_path[0])
        name = os.path.splitext(short_path)[0]


        classlabel = ntpath.dirname(image_path[0])
        classlabel = ntpath.basename(classlabel)
        ...
        for label, im_data in visuals.items():
            im = util.tensor2im(im_data)
            image_name = '%s_%s.png' % (name, label)
            save_path = os.path.join(image_dir, classlabel)
